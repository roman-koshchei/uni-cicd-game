...
class TestNode:
    ...
    def test_node_valid_directions(self):
        ...
        empty_valid_dirs = [
            direction
            for direction, neighbor in empty_node.neighbors.items()
            if neighbor is not None and direction != PORTAL
        ]
...
class TestNodeGroup:
    ...
    def test_get_node_methods(self):
        ...
        with patch("movement.nodes.NodeGroup.readMazeFile") as mock_read:
            ...
            node_group.getNodeFromTiles = lambda x, y: (
                node if x == 1 and y == 1 else None
            )

            # Replace the getNodeFromPixels method
            node_group.getNodeFromPixels = lambda x, y: (
                node if x == TILEWIDTH and y == TILEHEIGHT else None
            )
