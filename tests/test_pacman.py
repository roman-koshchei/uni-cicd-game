...
class TestPacman:
    def test_pacman_initialization(self, node, mock_pacman_sprites):
        ...
        # Mock the sprites
        with patch(
            "styles.sprite.sprites.PacmanSprites", return_value=MagicMock()
        ) as mock_sprites:
            pacman = Pacman(node)
    ...
    def test_pacman_key_pressed_handler(
        self, mock_get_pressed, connected_nodes, mock_pacman_sprites
    ):
        ...
        with patch(
            ...
            def mock_pressed():
                return type(
                    "obj",
                    (object,),
                    {"__getitem__": lambda self, k: key_states.get(k, False)},
                )()
